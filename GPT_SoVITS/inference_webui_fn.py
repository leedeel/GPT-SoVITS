...
def _process_reference_audio(vq_model,ref_wav_path, prompt_text, prompt_language, ref_free, 
    ...
    if not ref_free:
        ...
        with torch.no_grad():
            ...
            # 修正音频形状问题
            wav16k = _ensure_audio_shape(wav16k)
            
            ssl_content = ssl_model.model(wav16k)["last_hidden_state"].transpose(1, 2)
            codes = vq_model.extract_latent(ssl_content)
            prompt_semantic = codes[0, 0]
            prompt_semantic = prompt_semantic.unsqueeze(0).to(device)
    
    return prompt_semantic, zero_wav_torch


def _ensure_audio_shape(audio_tensor):
    """确保音频张量具有正确的形状"""
    print(f"音频原始形状: {audio_tensor.shape}")
    ...
    print(f"音频修正后形状: {audio_tensor.shape}")
    return audio_tensor
